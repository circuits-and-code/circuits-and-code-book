...
\subsection{Passives}
% This section introducing passives is very long, but it's critical information. Worst case we can move it to the appendix I guess, but I'd rather optimize this question's length in other methods. 
...
% This circuit can also be analyzed in the frequency domain. In this case for the capacitor $Z_c = \dfrac{1}{j \cdot \omega \cdot c}$ and for the resistor $Z_r = R$. For the voltage divider, $\dfrac{V_{out}}{V_{in}} = \dfrac{R_2}{R_1 + R_2}$, can be extrapolated into $\dfrac{V_{out}}{V_{in}} = \dfrac{Z_2}{Z_1 + Z_2}$ so by substituting in the values for this circuit $H = \dfrac{V_{out}}{V_{in}} = \dfrac{Z_c}{Z_r + Z_c} = \dfrac{\dfrac{1}{j \cdot \omega \cdot c}}{R + \dfrac{1}{j \cdot \omega \cdot c}} = \dfrac{1}{1 + j \cdot \omega \cdot c \cdot R} = \dfrac{1}{1+\dfrac{j \cdot \omega}{\omega_{0}}}$ where $\omega_{0} = \dfrac{1}{R \cdot C}$ is defined is a pole of the system transfer function $H$. Plotting this single pole low pass filter reveals:
% To find the step response using the Laplace domain $V_{out}(j \cdot \omega) = H(j \cdot \omega) \cdot \dfrac{1}{j \cdot w}$ is applied and then an inverse laplace transformation reveals an identical solution to the time domain solution.
