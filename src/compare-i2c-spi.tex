...
% more information about RC circuit diagram is not given here as it will be coverred in "passive_circuits.tex"
% should we add information about pull-up resistor value selection? 
% guide doesn't talk about UART at all anywhere, is that a concern?
% If we want to keep it to 20 questions, I've run out of space to talk about when to use an analog signal or a digital signal when doing system architecture, this definently comes up in interviews so idk if it's worth talking about.
...
\begin{itemize}
    \item \textbf{Bus Speed}: SPI is faster than I2C, with speeds in the MHz range compared to I2C's 400 kHz range due to the differences in drive type. As a result, SPI is often used in applications where the transfer speed is required to be high.
