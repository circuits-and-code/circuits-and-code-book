...
\noindent Another advantage of this configuration is immunity to common mode voltage offsets. Ideally CAN transceivers are designed to output a dominant state being CAN_H at 3.5V and CAN_L at 1.5V with a recessive state being CAN_H and CAN_L at 2.5V. However, an advantage of differential signalling is these can be shifted if two transceivers have a voltage potential differene between each of their ground planes so more accurately we can define a dominant state as CAN_H at $3.5 + N$ V and CAN_L at $1.5 + N$ V and a recessive state being  CAN_H and CAN_L at $2.5 + N$ V. Note that there is a limit to how large N can be based on the specific CAN transcievers being used. 
\noindent Shifts in ground potential are common in embedded systems with high current power transmission; consider a motor controller may be interfaced with CAN and be receiving power from a controller board. Due to the high current going to the motor and the parasitic resistance of the wire, a potential difference due to Ohm's law arrises between the ground on the controller and the ground on the motor controller. CAN is very robust to this small change in potential whereas a single ended signal could suffer logic level issues.
% maybe this is better explained in the signalling section to avoid bloating this? 
% might need a better explanation and some figures for this?
