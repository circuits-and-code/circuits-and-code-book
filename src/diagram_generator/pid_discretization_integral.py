...
def plot_sine_backward_derivative_and_sum(NUM_POINTS=20):
    # Time range
    T = 0.5 * np.pi  # One full sine wave period
    ...
    for i in range(1, NUM_POINTS + 1):
        ...
        # Plot the bottom line explicitly
        plt.plot(
            [t_discrete[i - 1], t_discrete[i]],
            [0, 0],
            color="blue",
    ...
    plt.plot(
        [], [], color="blue", linewidth=2, label="Backward Euler (Sum)"
    )  # Legend entry

    # Discrete points
    plt.scatter(t_discrete, y_discrete, color="red", zorder=6, label="Discrete Points")

    # Labels and styling
    plt.title("Sine Wave with Overlayed Backward Euler Sum Calculations")
    plt.xlabel("Time (t)")
    plt.ylabel("Value")
    plt.legend()
    plt.grid(True)
