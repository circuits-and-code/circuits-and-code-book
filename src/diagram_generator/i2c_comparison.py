...
def plotter():
    ...
    for i, t_i in enumerate(t):
        ...
        if cycle_pos < half_period:
            ...
            scl_higher[i] = v_ll * (
                1 - np.exp((-1 * (cycle_pos - half_period)) / tau_higher)
            )
            scl_lower[i] = v_ll * (
                1 - np.exp((-1 * (cycle_pos - half_period)) / tau_lower)
            )

    plt.figure(figsize=(10, 6))
    plt.title(f"{TITLE}")
    plt.plot(t, scl, label="SCL Nominal")
    plt.plot(t, scl_higher, label="SCL with Increased Pull-up Value")
    plt.plot(t, scl_lower, label="SCL with Decreased Pull-up Value")
    plt.xlabel("Time (sec)")
