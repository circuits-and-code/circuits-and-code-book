...
Assume the nominal voltage of the battery is 24 V, and the maximum input voltage of any integrated circuit (IC) or microcontroller is 3.3 V.
...
\begin{figure}[H]
    \begin{center}
        ...
        \end{circuitikz}
        % \setlength{\belowcaptionskip}{-10pt} % TODO : remove weird space after figure, idk how to do this correctly, need to figure it out eventually to save space on the page. - Use H to force figure to be placed here
...
The above assumption of no loading (i.e. no current drawn from) connected to $V_{out}$ is not always valid. In the case of an ADC, it may draw a small current from $V_{out}$ to sample the voltage, known as \textit{input bias current}. Note this may also be represented by input impedance in which the load current is instead modelled by a loading resistor, however, the concept remains similar. This input bias current, $I_{bias}$, is often in the micro-amp range and is used to feed the internal analog circuit and is present to varying degrees of severity in all forms of ADCs. As load current is added to the output of the voltage divider, the voltage at $V_{out}$ will drop and diverge from the expected $V_out$ value calculated in \eqref{eq:voltage_divider}.
...
\noindent Op-amps also have input bias current, however, we can compensate for it using the following circuit:
