...
\noindent Using the circuit in Figure \ref{ct:voltage_divider_loaded} we can solve for a new equation for $V_{out}$ shown in xxx. Input bias current is also hard to model and can vary significantly so the simplest method of compensating for this by reducing $R_{sum}$ to increase $I_{R_t}$. \newline
