...
% An example of an excessively complex nonsenical solution an interviewee could propose is using an ADC that supports rlly high input voltages such as ADS125H01 https://www.ti.com/product/ADS125H01 . The question is meant to leave bait for something like that
...
% TODO : explain why a separate high voltage ADC is stupid here
...
\begin{figure}[H]
    \begin{center}
        \begin{circuitikz}[american]
            ...
            \label{figure:voltage_divider_loaded}
        \end{circuitikz}
...
\noindent Using the circuit in Figure \ref{figure:voltage_divider_loaded} we can solve for a new equation for $V_{out}$ shown in xxx. Input bias current is also hard to model and can vary significantly so the simplest method of compensating for this by reducing $R_{sum}$ to increase $I_{R_t}$. \newline
...
\begin{figure}[H]
    \begin{center}
        ...
        \end{circuitikz}
        \caption{Op-amp Bufferred Resistor Divider}
    \end{center}
\end{figure}
...
\begin{figure}[H]
    \begin{center}
        \begin{circuitikz}[american]
            ...
            \draw (1.5, 0) node[ground]{};
            \label{figure:voltage_divider_low_passed}
        \end{circuitikz}
...
% Moved to be a follow-up question instead for now. 
...
%             \label{figure:voltage_divider_switched}
%         \end{circuitikz}
