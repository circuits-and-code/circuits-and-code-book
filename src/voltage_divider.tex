...
\noindent Using the circuit in Figure \ref{fig:voltage_divider_loaded} results in an adjusted transfer function shown in Equation \ref{eq:loaded_voltage_divider}. Input bias current is also hard to model and can vary significantly so the simplest method of compensating for this is by reducing $R_{sum} = R_t + R_b$ while maintainning the same result for $\dfrac{R_b}{R_t + R_b}$. \label{statement:input_bias_compensated_voltage_divider} \newline
...
\begin{itemize}
    ...
    \item Why does \ref{statement:input_bias_compensated_voltage_divider} work? % Algebraic proof as I showed in my commented formulas 
\end{itemize}
